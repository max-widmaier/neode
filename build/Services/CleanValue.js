    ...
    case 'point':
      ...
      if (isNaN(value.x)) {
        ...
        if (isNaN(value.height)) {
          value = new _neo4jDriver["default"].types.Point(4326,
          // WGS 84 2D
          value.longitude, value.latitude);
        } else {
          value = new _neo4jDriver["default"].types.Point(4979,
          // WGS 84 3D
          value.longitude, value.latitude, value.height);
        }
      } else {
        if (isNaN(value.z)) {
          value = new _neo4jDriver["default"].types.Point(7203,
          // Cartesian 2D
          value.x, value.y);
        } else {
          value = new _neo4jDriver["default"].types.Point(9157,
          // Cartesian 3D
